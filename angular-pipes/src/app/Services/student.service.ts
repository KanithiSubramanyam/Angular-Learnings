...
export class StudentService{
    ...
    filterStudentByGender(filterGender : string) {
        if(filterGender.toLowerCase() === 'all' || this.students.length === 0 || filterGender === null || filterGender === '') 
            return this.students;
        else
            return this.students.filter(x => x.gender.toLowerCase() === filterGender.toLowerCase());
    }
    }
